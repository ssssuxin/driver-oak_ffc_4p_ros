...
DOCKERIMAGE="hkustswarm/ros_oak_quad_cam:latest"
if [ $# -eq 0 ]; then
...
if [ ${START_OPTION} == 1 ]; then
  ...
  docker run -it --name=oak --net=host -v ${CURRENT_DIR}:/root/oak_ffc_ws/src/oak_ffc_4p_ros -v /dev/:/dev/  --privileged -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix  ${DOCKERIMAGE} /bin/bash 
else
  echo "Start docker container for image transportation only"
  docker run -it --name=oak --net=host -v /dev/:/dev/  --privileged -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix  ${DOCKERIMAGE} /bin/bash
fi
